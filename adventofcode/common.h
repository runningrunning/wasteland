...
struct DListNode
{
    ...
    struct DListNode* nxt;
};

struct DListNode* cDListNode(int val)
{
    struct DListNode* n = malloc(sizeof(struct DListNode));
    n->val = val;
    n->pre = n->nxt = NULL;
    return n;
}
...
long* gen_hash_l(int min_length, int* prime)
{
    ...
    long* hash = calloc(sizeof(long), dp);
    *prime = dp;
    return hash;
}
