...
void findSum(int* cs, int size, int target, int* result, int base, int ***all, int** cSize, int* rSize, int *rLimit )
{
    ...
    int skip = 1;
    while((skip < size) && cs[skip] == t) skip ++;
    if (skip < size)
        findSum(&cs[skip], size - skip, tg, result, base + ri, all, cSize, rSize, rLimit);
...
int** combinationSum2(int* candidates, int candidatesSize, int target, int** columnSize, int* returnSize)
{
    if (!candidatesSize)
    {
        *columnSize = NULL;
